...
if ! rg -n 'redirect\("/dashboard"\)' "$HUB_PAINEL" >/dev/null 2>&1 && ! rg -n 'from "\.\./dashboard/page"' "$HUB_PAINEL" >/dev/null 2>&1; then
...
rg -n 'loginUrl\.pathname\s*=\s*"/login"' "$MW" >/dev/null || { echo "middleware must redirect unauth to /login" >&2; exit 1; }
rg -n 'matcher:\s*\["/:path\*"\]' "$MW" >/dev/null || { echo "middleware matcher must cover routes" >&2; exit 1; }
