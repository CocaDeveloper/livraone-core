...
rg -q '^- NEXTAUTH_URL$' "$SECRETS_DOC" || fail "NEXTAUTH_URL not documented in SSOT contract"
rg -q '"/api/auth"' "$MIDDLEWARE" || fail "middleware must allow /api/auth"
rg -q 'HUB_AUTH_CALLBACK_URL=https://hub.livraone.com/api/auth/callback/keycloak' "$ENV_EXAMPLE" || fail "env.example must include hub auth callback URL"
