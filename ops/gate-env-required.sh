...
if [ ! -f "$ENVFILE" ]; then
  ...
  cat <<MSG >&2
.env missing in $ROOT. Create it with:
  ACME_EMAIL="you@example.com"
  CF_API_TOKEN="your-cloudflare-token"
Set strict permissions (chmod 600 .env) and rerun.
MSG
  exit 1
fi
...
if ! $PASS; then
  echo "GATE-ENV-REQUIRED=FAIL"
  cat <<MSG >&2
$ENVFILE lacks required keys. Inspect $EVIDENCE/env-check.txt for details.
MSG
  exit 1
fi
