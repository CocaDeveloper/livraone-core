...
## TLS and DNS-01
Traefik uses `certificatesResolvers.cloudflare` to request certificates via Let’s Encrypt DNS-01 challenges. Tokens are injected through the hub environment file (mirroring the `env.example` variables `CF_API_TOKEN` and `ACME_EMAIL`), with credentials never checked into git.
